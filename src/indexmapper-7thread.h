...
//            sqlite3_close(db);
        ...
        void insertBlockData(vector<json>  blockData){
            ...
            if( rc == SQLITE_OK ) {
                for(int x=0;x<blockData.size();x++){
                    // bind the value
                    std::string id = blockData[x].at("lastBlockId");
                    sqlite3_bind_text(stmt, 1, id.c_str(),strlen(id.c_str()), 0);
                    ...
                    {
                        printf("Commit block data Failed! %d\n", retVal);
                    }
        ...
        void insertBlockIndex(vector<std::vector<string>> blockIndex){
            sqlite3_mutex_enter(sqlite3_db_mutex(db));
            ...
            if( rc == SQLITE_OK ) {
                for(int x=0;x<blockIndex.size();x++){
                    // bind the value
                    sqlite3_bind_text(stmt, 1, blockIndex[x][0].c_str(),strlen(blockIndex[x][0].c_str()),0);
                    sqlite3_bind_text(stmt, 2, blockIndex[x][0].c_str(),strlen(blockIndex[x][0].c_str()),0);
                    sqlite3_bind_text(stmt, 3, blockIndex[x][0].c_str(),strlen(blockIndex[x][0].c_str()),0);

                    // commit
                    int retVal = sqlite3_step(stmt);
                    if (retVal != SQLITE_DONE)
                    {
                        printf("Commit block index Failed! %d\n", retVal);
                    }
        ...
        void insertSAData(vector<json> saData){
            ...
            if( rc == SQLITE_OK ) {
                for(int x=0;x<saData.size();x++){
                    // bind the value
                    std::string id = saData[x].at("lastSAId");
                    sqlite3_bind_text(stmt, 1,  id.c_str(), strlen(id.c_str()), 0);
                    ...
                    if (retVal != SQLITE_DONE)
                    {
                        printf("Commit sa data Failed! %d\n", retVal);
                    }
        ...
        void insertSAIndex(vector<std::vector<string>> saIndex){
            sqlite3_mutex_enter(sqlite3_db_mutex(db));
            ...
            if( rc == SQLITE_OK ) {
                for(int x=0;x<saIndex.size();x++){
                    // bind the value
                    sqlite3_bind_text(stmt, 1, saIndex[x][2].c_str(),strlen(saIndex[x][2].c_str()),0);
                    sqlite3_bind_text(stmt, 2, saIndex[x][3].c_str(),strlen(saIndex[x][3].c_str()),0);
                    sqlite3_bind_text(stmt, 3, saIndex[x][0].c_str(),strlen(saIndex[x][0].c_str()),0);
                    sqlite3_bind_text(stmt, 4, saIndex[x][1].c_str(),strlen(saIndex[x][1].c_str()),0);

                    // commit
                    int retVal = sqlite3_step(stmt);
                    if (retVal != SQLITE_DONE)
                    {
                        printf("Commit sa index Failed! %d\n", retVal);
                    }
        ...
        void insertKIData(vector<std::vector<string>> kiData){
            ...
            if( rc == SQLITE_OK ) {
                for(int x=0;x<kiData.size();x++){
                    // bind the value
                    sqlite3_bind_text(stmt, 1, kiData[x][0].c_str(), strlen(kiData[x][0].c_str()), 0);
                    sqlite3_bind_text(stmt, 2, kiData[x][1].c_str(), strlen(kiData[x][1].c_str()), 0);
                    sqlite3_bind_text(stmt, 3, kiData[x][2].c_str(), strlen(kiData[x][2].c_str()), 0);
                    sqlite3_bind_text(stmt, 4, kiData[x][3].c_str(), strlen(kiData[x][3].c_str()), 0);

                    // commit
                    int retVal = sqlite3_step(stmt);
                    if (retVal != SQLITE_DONE)
                    {
                        printf("Commit key-image data Failed! %d\n", retVal);
                    }
        ...
        void insertRMData(vector<json>  rmData){
            ...
            if( rc == SQLITE_OK ) {
                for(int x=0;x<rmData.size();x++){
                    // bind the value
                    std::string id = rmData[x].at("lastRingId");
                    sqlite3_bind_text(stmt, 1,  id.c_str(), strlen(id.c_str()), 0);
                    ...
                    if (retVal != SQLITE_DONE)
                    {
                        printf("Commit rm data Failed! %d\n", retVal);
                    }
        ...
        void insertRMIndex(vector<std::vector<string>> rmIndex){
            sqlite3_mutex_enter(sqlite3_db_mutex(db));
            ...
            if( rc == SQLITE_OK ) {
                for(int x=0;x<rmIndex.size();x++){
                    // bind the value
                    sqlite3_bind_text(stmt, 1, rmIndex[x][0].c_str(),strlen(rmIndex[x][0].c_str()),0);
                    sqlite3_bind_text(stmt, 2, rmIndex[x][4].c_str(),strlen(rmIndex[x][4].c_str()),0);
                    sqlite3_bind_text(stmt, 3, rmIndex[x][1].c_str(),strlen(rmIndex[x][1].c_str()),0);
                    sqlite3_bind_text(stmt, 4, rmIndex[x][2].c_str(),strlen(rmIndex[x][2].c_str()),0);
                    sqlite3_bind_text(stmt, 5, rmIndex[x][3].c_str(),strlen(rmIndex[x][3].c_str()),0);

                    // commit
                    int retVal = sqlite3_step(stmt);
                    if (retVal != SQLITE_DONE)
                    {
                        printf("Commit rm index Failed! %d\n", retVal);
                    }
        ...
        void insertTxIndex(vector<std::vector<string>> txIndex){
            sqlite3_mutex_enter(sqlite3_db_mutex(db));
            ...
            if( rc == SQLITE_OK ) {
                for(int x=0;x<txIndex.size();x++){
                    // bind the value
                    sqlite3_bind_text(stmt, 1, txIndex[x][0].c_str(),strlen(txIndex[x][0].c_str()),0);
                    sqlite3_bind_text(stmt, 2, txIndex[x][1].c_str(),strlen(txIndex[x][1].c_str()),0);
                    sqlite3_bind_text(stmt, 3, txIndex[x][2].c_str(),strlen(txIndex[x][2].c_str()),0);

                    // commit
                    int retVal = sqlite3_step(stmt);
                    if (retVal != SQLITE_DONE)
                    {
                        printf("Commit tx index Failed! %d\n", retVal);
                    }
        ...
        void insertTxData(vector<json>  txData){
            ...
            if( rc == SQLITE_OK ) {
                for(int x=0;x<txData.size();x++){
                    ...
                    std::string id = txData[x].at("lastTxId");
                    sqlite3_bind_text(stmt, 1, id.c_str(),strlen(id.c_str()), 0);
                    ...
                    if (retVal != SQLITE_DONE)
                    {
                        printf("Commit tx data Failed! %d\n", retVal);
                    }
